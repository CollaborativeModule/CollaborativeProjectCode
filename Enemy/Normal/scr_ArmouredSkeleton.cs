...
public class scr_ArmouredSkeleton : MonoBehaviour, scr_IBaseEnemy
{
    ...
    public void Update()
    {
        ...
        if (inChaseRange == true && inAttackRange == false && isExhausted == false)
        {
            ...
            // plays enemy walk sound if not currently played
            if (walkSoundPlayed == false)
            {
                walkSoundPlayed = true;
                scr_AudioManager.instance.armouredSkeletonEnemy.setParameterByName("Enemies_State", 1f);
                scr_AudioManager.instance.armouredSkeletonEnemy.start();
            }

            // enemy walk sound delay, if played
            else
            {
        ...
        else if (inChaseRange == false && inAttackRange == false && isExhausted == false)
        {
            ...
            // plays enemy idle sound if not currently played
            if (idleSoundPlayed == false)
            {
                idleSoundPlayed = true;
                scr_AudioManager.instance.armouredSkeletonEnemy.setParameterByName("Enemies_State", 0f);
                scr_AudioManager.instance.armouredSkeletonEnemy.start();
            }

            // enemy idle sound delay, if played
            else
            {
    ...
    public void Chase()
    {
        // moves enemy towards, the player position smoothly
    ...
    public void Attack()
    {
        // random enemy move and associated value's/actions
        float randomMove = Random.Range(0, 2);
        if (randomMove == 0)
        {
            enemy.GetComponent<scr_BasicMeleeAttack>().Melee(baseDamage, inAttackRange);
            scr_AudioManager.instance.armouredSkeletonEnemy.setParameterByName("Enemies_State", 2f);
            scr_AudioManager.instance.armouredSkeletonEnemy.start();
        ...
        {
            scr_AudioManager.instance.armouredSkeletonEnemy.setParameterByName("Enemies_State", 3f);
            scr_AudioManager.instance.armouredSkeletonEnemy.start();
            Guard();
        }
    }

    public void Guard() // plays animation and triggers a variable that reduces damage taken
    {
        animator.SetTrigger("Guarding");
        enemy.GetComponent<scr_Guard>().isGuarding = true;
        Invoke("UnGuard", 0.5f);
    }

    public void UnGuard() // allows enemy to do the attack function and returns damage taken back to normal levels
    {
        enemy.GetComponent<scr_Guard>().isGuarding = false;
        hasAttacked = false;
    }
